...
| `apps/server/src/mcp/McpHttpServer.ts`                   | `multilinearMcpLayer`                                     | Registers the `ml_*` MCP toolkit on upstream's `/mcp` server (import + merge into the exported layer)                                   | 2026-07-04 |
| `apps/server/src/provider/CodexDeveloperInstructions.ts` | `MULTILINEAR_DEVELOPER_INSTRUCTIONS`                      | Appends the multilinear tracker steering block to both Codex mode templates (import + two interpolations, MLT-56)                       | 2026-07-04 |
