...
| D24 | 2026-07-05. Live updates (MLT-63) are client polling of a new `events.head` query (`MAX(seq)` over the event log, 2s interval, paused while the tab is hidden) — the signal is derived from the database on purpose, because writes also arrive from other processes (stdio MCP agents), which an in-process bus/SSE-off-the-store would miss. SSE/WS rejected: D11 keeps us off the WS RPC conflict surface, and connection lifecycle buys nothing for a local single-user board. Revisit if latency ever matters.                                                                                                               | RESOLVED (agent)                               |
| D25 | 2026-07-05. Space→repo mapping (MLT-47/51) lives in the tracker as an event-sourced Space property (`space.update` → `space.updated`, `repoPaths` of absolute repo roots, projection schema v4): multi-repo spaces need one authoritative list, the Phase 3 dispatcher reads it from the store, and repo-side declaration would force the server to discover/scan repos. Repo-owned `.multilinear/` keeps the _content_ (profiles, rules) per 01 §4; the tracker keeps the _pointer_. Profile loading unions the walk-up default dir with every mapped repo's `.multilinear/profiles`; earliest dir wins name collisions, loudly. | RESOLVED (agent)                               |
...
**Done — all 4 slate items (5 issues), each claimed → in_progress → proof →
needs_review through the tracker as agent `claude-code:quick-wins-2`, one
commit per issue (one for the 47+51 pair):**

- **MLT-64** board contrast: canvas < column < card in both themes — the
  theme's `--card`/`--muted` sit within ~2% of `--background`, so cards get
  an explicit elevated fill (shadow on light, white/7% on dark) and columns
  a recessed tint. Board-component classes only.
- **MLT-63** live updates (D24): `events.head` change probe (MAX(seq)) in
  core/router; the client polls every 2s while visible via a shared
  subscriber module and refetches on movement — DB-derived so stdio-MCP
  writes from other processes are seen (verified live cross-process).
  Detail refetches silently; title drafts survive background refreshes.
- **MLT-47 + MLT-51** space→repo mapping (D25): event-sourced `repoPaths`
  on Space (`space.update`, projection v4), Repositories dialog on space
  rows, Start-agent routes drafts into the matching upstream project
  (browser-verified), profiles load from every mapped repo's
  `.multilinear/profiles` via a multi-directory loader re-resolved each
  rescan (collisions loud, earliest dir wins). MLT is now really mapped to
  this repo.
- **MLT-48** Linear-style view controls, two passes — the human shared
  Linear's menus mid-session and the controls were rebuilt to match:
  searchable Filter menu (priority/status/label multi-select) + removable
  filter chips; Display popover owning the List|Board toggle, ordering +
  direction, list grouping, show-empty switches, property pills, Reset;
  all persisted in localStorage; keyboard nav follows the visible view.

**Verification bonus — a real bug caught and fixed (36bf73748):** the
browser pass found repo-mapping saves failing with `apply-event` errors.
Root cause: the projection migration's DROP+CREATE auto-commit per
statement, and the RUNNING OLD-CODE Alpha instance sharing
`~/.multilinear/tracker.db` recreated `spaces` in the old shape between
our DROP and CREATE via its open-time `CREATE TABLE IF NOT EXISTS` — with
`user_version` already advanced. Fix: migration is one `BEGIN IMMEDIATE`
transaction, plus a shape-drift check on every open (normalized
`sqlite_master` vs `SCHEMA_SQL`) that forces drop-and-replay — which also
healed the live DB. Regression test simulates the exact corrupt state.
**Operational note: restart the Alpha t3code instance after pulling — its
in-memory code predates projection v4 and it shares the tracker DB.**

**Gate:** merged `upstream/main` (1 commit, favicon only — tripwire clear);
`vp check` 0 errors; repo typecheck clean; 84 core + 24 server + 10 web
tests green. Browser verification: all checks pass (contrast, filters/
