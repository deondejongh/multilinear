...
t3code *or* a raw Claude Code terminal — can read issues, comment, transition
...
Allowed: `triage→backlog`, `backlog→ready`*, `ready→in_progress`,
...
(*`backlog→ready` only when the issue passes the Ready gate, §E.)
...
`trust_tier` (read_only | workspace_write | network), `allowed_transitions`
...
*recorded and surfaced* now; enforcement beyond t3code's own controls is Phase 4+.
...
## Out of scope
Auto-dispatch, schedules, rules, GitHub sync, trust-tier *enforcement*, tournament.
